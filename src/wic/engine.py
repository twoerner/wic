...
def verify_build_env(vars_dir=None):
    """
    ...
    """
    if vars_dir:
        if os.path.isdir(vars_dir) or os.path.isfile(vars_dir):
            return True
        raise WicError("The supplied vars path %s does not exist" % vars_dir)
...
def wic_create(wks_file, rootfs_dir, bootimg_dir, kernel_dir,
    ...
    oe_builddir = get_bitbake_var("TOPDIR")
